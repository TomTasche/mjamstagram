...
public class PhotoServiceManager {
	private static final Logger logger = Logger
			.getLogger(PhotoServiceManager.class.getCanonicalName());
	private ConfigManager configManager;
	private PhotoManager photoManager;

	public PhotoServiceManager(ConfigManager configManager,
	...
	public String getUploadUrl() {
		...
		return blobstoreService.createUploadUrl(
				configManager.getUploadHandlerUrl(), uploadOptions);
	}
	...
	public String getMenuLinkUrl(String targetUrl, Photo photo) {
		...
		return new StringBuilder(targetUrl).append("?")
				.append(ServletUtils.REQUEST_PARAM_NAME_PHOTO_ID).append("=")
				.append(photo.getId()).append("&")
				.append(ServletUtils.REQUEST_PARAM_NAME_PHOTO_OWNER_ID)
				.append("=").append(photo.getOwnerId()).toString();
	}

	public String getImageDownloadUrl(Photo photo) {
		return new StringBuilder(configManager.getDownloadHandlerUrl())
				.append("?").append(ServletUtils.REQUEST_PARAM_NAME_PHOTO_ID)
				.append("=").append(photo.getId()).append("&")
				.append(ServletUtils.REQUEST_PARAM_NAME_PHOTO_OWNER_ID)
				.append("=").append(photo.getOwnerId()).toString();
	}

	/**
	 * Constructs a redirect url to specific photo. if the photo information is
	 * not available, return to the main page.
	 * 
	 * @param targetUrl
	 *            target url. If null,
	 * @param userId
	...
	private void removeDeactivedPhoto(Photo photo) {
		if (photo != null && !photo.isActive()) {
			try {
				...
				if (stat != null) {
					logger.fine("photo:" + photo.getId()
							+ " blob file stat is not null");
					BlobstoreService blobstoreService = BlobstoreServiceFactory
							.getBlobstoreService();
				...
				logger.severe("Failed to delete the blob storge for photo "
